...
#######################################################################
# data
# read text and clean it up a bit
# create a dictionary and reverse lookup dictionary
#######################################################################

# only need to run this once to clean up original text
def clean_data():
  
  data = [] 
  with open('BothBooks.txt', encoding='utf-8') as f:
    for line in f:
        data.append(line)
  ...
  with open('cleaned_alice.txt', 'a') as myfile:
        for t in data:
            myfile.write(str(t))
...
####################################################################################
def plot_with_labels(low_dim_embs, labels, filename='alice_word_vectors.png'):
